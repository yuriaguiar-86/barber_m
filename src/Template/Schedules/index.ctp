                        ...
                        <?php if ($schedule->finished != FinishedENUM::FINISHED) : ?>
                            <td class="actions">
                                ...
                                <?php if ($this->AppView->visible('Schedules', 'delete')) : ?>
                                    <div class="delete">
                                        <?= $this->Form->postLink(__('<i class="fa-solid fa-trash"></i> Cancelar'), ['controller' => 'Schedules', 'action' => 'delete', $schedule->id], ['class' => 'action__delete sweetdelete', 'data-name' => 'o agendamento do dia ' . $schedule->date->format('d/m/Y') . ' ás ' . $schedule->time . ':00H', 'escape' => false, 'confirm' => __('Tem certeza que deseja cancelar o agendamento do dia {0}?', $schedule->date->format('d/m/Y'))]); ?>
                                    </div>
                                <?php endif; ?>
                                        ...
                                        <?php if (
                                            ...
                                            <li><a href="#" class="header__link"><i class="fa-solid fa-ellipsis"></i></a>
                                                <ul class="dropdown">
                                                    ...
                                                    <?php if ($this->AppView->visible('Schedules', 'delete')) : ?>
                                                        <li><?= $this->Form->postLink(__('Apagar'), ['controller' => 'Schedules', 'action' => 'delete', $schedule->id], ['class' => 'header__link sweetdelete', 'data-name' => 'o agendamento do dia ' . $schedule->date->format('d/m/Y') . ' ás ' . $schedule->time . ':00H', 'escape' => false, 'confirm' => __('Tem certeza que deseja cancelar o agendamento do dia {0}?', $schedule->date->format('d/m/Y'))]); ?></li>
                                                    <?php endif; ?>
                                                </ul>
                                            </li>
                                        <?php endif; ?>
